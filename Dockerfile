...
## Install DeepEP
# -----------------------------
# Python: NVSHMEM + PyTorch (cu12x) + test tooling
# -----------------------------
...
# -----------------------------
# DeepEP sources (pin at build time for repeatability)
# -----------------------------
ARG DEEPEP_REPO=https://github.com/deepseek-ai/DeepEP.git
# Optionally pin to a commit: --build-arg DEEPEP_REF=<git-sha-or-tag>
ARG DEEPEP_REF=main
RUN git clone --depth 1 --branch ${DEEPEP_REF} ${DEEPEP_REPO} /opt/DeepEP-src

# Build DeepEP against NVSHMEM wheel (DeepEP's setup finds the wheel via importlib.resources)
WORKDIR /opt/DeepEP-src
RUN python3 -m pip install -v -e .  # builds CUDA extensions with SM90-only targets
